      ...
      {/* <div className="rounded border border-gray-500 p-4 group-hover:border-green-500 group-active:bg-green-500">
        <header className="flex items-center justify-between"> */}
          {isLessonAvaible ? (
            ...
            <span className="text-xs text-white border  border-green-500 group-active:border-white rounded py-[2px] px-2 font-bold">
            {props.lessonType == "live" ? "AO VIVO" : "AULA PRÁTICA"}
          </span>
        </header>
        <strong className="text-gray-200 group-active:text-white mt-5 block">{props.title}</strong>
      </div>
          ) : (
            <div className="rounded border border-gray-500 p-4 group-hover:border-orange-500 group-active:bg-orange-500 ">
            <header className="flex items-center justify-between">
            <span className="text-sm text-orange-500 font-medium flex items-center gap-2  group-active:text-white">
              <Lock size={20} />
              Em Breve
            </span>
            <span className="text-xs text-white border  border-orange-500 group-active:border-white rounded py-[2px] px-2 font-bold">
            {props.lessonType == "live" ? "AO VIVO" : "AULA PRÁTICA"}
          </span>
        </header>
        <strong className="text-gray-200 group-active:text-white mt-5 block">{props.title}</strong>
